...
| Job | Triggers | Notes |
|---|---|---|
| `changes` | every event | `dorny/paths-filter` — doc-only PRs (anything matching `**.md`, `docs/**`, `LICENSE`, etc., except `CLAUDE.md` which is re-included) skip every job below. `base: ${{ github.event_name == 'push' && 'master' \|\| '' }}` to handle act + PR-event annotation cases. Also emits a `docs` output (matches `README.md`) for the `mermaid-lint` job. |
| `mermaid-lint` | README-only changes (`code != true && docs == true`) | Cheap docker + `minlag/mermaid-cli` validation of the README C4 hero diagram (`make mermaid-lint`, no Maven/mise). README is `**.md` so a README-only edit skips `build` — without this job its `make ci`-embedded mermaid-lint would never run on the change. Stays idle (skipped) on code/tag events, where `build`'s `make ci` validates the diagram. |
...
| `cve-check` | tag pushes + weekly cron + dispatch | OWASP dependency-check via `mvn org.owasp:dependency-check-maven:check` (NVD + Sonatype OSS Index analyzers). NVD DB cached at `~/.m2/repository/org/owasp/dependency-check-data` keyed on the **ISO week** (the DB is independent of `pom.xml`, so a version bump must not evict it). **`NVD_API_KEY` is strongly recommended** — without it, anonymous NVD rate-limiting exhausts the plugin's parallel connection pool and the job fails with `commons-dbcp2.BasicDataSource.getConnection() — connectionPool is null` on cold fetches (empirically verified 2026-05-31 across multiple tag runs). Free API key from <https://nvd.nist.gov/developers/request-an-api-key>; routed via `~/.m2/settings.xml`, never argv. |
...
| `ci-pass` | always | Single branch-protection aggregator over all six other jobs. Treats skipped as pass (handles tag-only `docker`/`cve-check`, master-or-tag-only `release`, README-only `mermaid-lint`, doc-only-PR-skipping everything). |
...
- **JCommander is on the frozen `com.beust:jcommander:1.82` coordinate; the maintained line moved to `org.jcommander:jcommander` (now 3.0) (LOW priority, no CVE).** `com.beust:jcommander` was last published 2022-01-11 and is frozen at 1.82 — so Renovate's `maven` manager reports the pin "current" forever (nothing newer exists under that groupId). This is a coordinate-rename blind spot (`/upgrade-analysis` §5c), not a genuine "up to date": the project is 2 majors behind on an unmaintained artifact. **Trigger to act:** a CVE disclosed against jcommander 1.82, or any need to touch CLI parsing. **Action (NOT a drop-in — verify, don't just bump):** swap the dependency to `org.jcommander:jcommander:3.0`, recompile, and fix any breaks in `ExtCommander.java` (it `extends JCommander` and its inner `ExtUsageFormatter extends DefaultUsageFormatter` overrides `usage(StringBuilder, String)` — that exact internal API already churned at 1.82 when usage moved to `IUsageFormatter`, and may have changed again in 2.x/3.x) and `CLIArguments.java` (the "`@Parameter` fields must not be `final`" 1.82 quirk may differ in 3.x). Verify with the 9 CLI-exercising tests (`LdapServerTest`, `CustomPasswordTest`, `AnonymousBindTest`, `StartTlsTest`). The Java package likely stays `com.beust.jcommander` across the groupId rename, but confirm before assuming imports are unchanged. Effort: Minor if `DefaultUsageFormatter` is unchanged, Major if it was reworked.
